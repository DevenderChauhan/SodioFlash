...
public class PersonalPresenter extends BasePresenter<MvpView> {
    ...
    public PersonalPresenter(PersonalActivity activity) {
    ...
    public void upIcon(Bitmap imgUrl){
        if (imgUrl != null){
            addSubscription(fileController.postFile(activity, imgUrl).
                    subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(new Action1<String>() {
                        @Override
                        public void call(String s) {
                            addSubscription(memberController.saveUserHead(s)
                                    .subscribeOn(Schedulers.io())
                                    .observeOn(AndroidSchedulers.mainThread())
                                    .subscribe(new Action1<BaseBean>() {
                                        ...
                                        public void call(BaseBean bean) {
                                            if (bean.getError_code() == 0){
                                                activity.upIconSuccess();
                                                Toast.makeText(activity,activity.getString(R.string.person_modify_success),Toast.LENGTH_SHORT).show();
                                            }else{
            ...
            Toast.makeText(activity,activity.getString(R.string.person_modify_failed),Toast.LENGTH_SHORT).show();
        }
